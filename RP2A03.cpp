...
void RP2A03::reset() {
    ...
    for (uint8_t i=0; i<5; i++) {
    	memset(&chn[0]+i, 0x00, sizeof(channel) );
...
int RP2A03::tick(uint8_t c) {
	channel& ch = chn[c];
	if (!ChannelsEnabled[c]) return c<4?8:ch.linear_counter;
    ...
    case 3:	if(!ch.hold) ch.hold = 1;
		...
		case 4: if(ch.phase == 0) {
	                ...
	                if(ch.length_counter > 0) {
	                    ...
	                    ch.phase = 8;
	                    --ch.length_counter;
	                } else ChannelsEnabled[c] = ch.reg3&0x80 && (DMC_IRQ = true);
	            }
	            if(ch.phase != 0) {
...
uint8_t	 RP2A03::MemAccess(uint16_t addr, uint8_t data, bool write) {
	...
	switch (addr<0x10 ? addr%4 : addr) {
		...
		case 3: if (!write) break;
						...
						if(ChannelsEnabled[addr>>2]) ch.length_counter = LengthCounters[ch.reg3>>3];
		...
		case 0x10: if (!write) break;
						...
						break;
		case 0x11: if (write) direct_output = data&0x7F; break;
		...
		case 0x15:	if (write) {
								...
								for(uint8_t c=0; c<5; c++) {
									ChannelsEnabled[c] = data & (1 << c);
									if(!ChannelsEnabled[c]) chn[c].length_counter = 0;
									else if(c == 4 && chn[c].length_counter == 0)
									chn[c].length_counter = (chn[c].reg1 << 4) | 1;
								}
							} else {
								result = 0x00;
								for(uint8_t c=0; c<5; c++) result |= (chn[c].length_counter ? 1 << c : 0);
		...
		case 0x17: if (!write) break;
								FiveCycleDivider = data & 0x80;
								IRQdisable       = data & 0x40;
								hz240counter     = { 0,0 };
								if(IRQdisable) Frame_IRQ = DMC_IRQ = false;
								break;
	}
	return result;
}
...
bool RP2A03::getAudioReady(unsigned SkeepBuffer) {
	return ((hz240counter.lo % SkeepBuffer) < 2);
}
...
void RP2A03::clock() {
	if((hz240counter.lo += 2) >= 14915) {
	  ...
	  if(!IRQdisable && !FiveCycleDivider && hz240counter.hi==0) Frame_IRQ = true;
  ...
  +  v(159.79f, (100.f + v(1.0, s[2]/8227.f + s[3]/12241.f + s[4]/22638.f + direct_output/22638.f, -100.f)), 0.f)
    - 0.5f
    );	
}